...
BASE = '/tmp/claude-0/-home-user/f7826394-e5e3-5463-a537-6672931e9cc4/scratchpad/newlisting'
D = os.path.join(BASE, 'data')
STOP_SLIP = 0.0      # verifier: extra adverse slippage on stop fills (capped at the bar extreme)
STOP_WORST = False   # verifier: stop fills at the bar extreme (high for shorts) instead of max(open, stop)
HMS = 3600000
...
class Data:
    def __init__(self, price='hybrid'):
        ...
        if price == 'hybrid':
            ...
            self.src_okx = has
        if price == 'hybrid2':   # verifier: hybrid + OKX OHLC rebuilt from OKX trade archives for events without REST candles
            Q = np.load(os.path.join(D, 'panel_okx.npz'))
            Qo, Qh, Ql, Qc = (Q[k].copy() for k in 'ohlc')
            for f in ('okx_rebuilt_h1_oos.parquet', 'okx_rebuilt_h1_is.parquet'):
                fp = os.path.join('/tmp/claude-0/-home-user/f7826394-e5e3-5463-a537-6672931e9cc4/scratchpad/verify_listings', f)
                if not os.path.exists(fp):
                    continue
                R = pd.read_parquet(fp)
                for sym, g in R.groupby('sym'):
                    ii = np.where(self.ev.sym.values == sym)[0]
                    for i in ii:
                        k = ((g.t.values - self.ev.t0.values[i]) // HMS).astype(np.int64)
                        ok = (k >= 0) & (k < self.o.shape[1]) & ((g.t.values - self.ev.t0.values[i]) % HMS == 0)
                        fac = g.factor.values[ok]
                        for a, arr in (('o', Qo), ('h', Qh), ('l', Ql), ('c', Qc)):
                            arr[i, k[ok]] = g[a].values[ok] * fac
            has = np.isfinite(Qo) & np.isfinite(Qh) & np.isfinite(Ql) & np.isfinite(Qc)
            self.o, self.h, self.l, self.c = (np.where(has, q, x) for q, x in ((Qo, self.o), (Qh, self.h), (Ql, self.l), (Qc, self.c)))
            self.src_okx = has
...
def simulate(Dt, cfg, start=IS_START, end=OOS_START, L=1.0, cost_mult=1.0, lat=0, exclude=None,
             min_frac=0.0, record=False, excl_months=None, slip_coin=None):
    ...
    fee_c = (FEE + (SLIP_COIN if slip_coin is None else slip_coin)) * cost_mult
