...
def wick_sim(o, h, l, c, mh, ml, mc, fund, fflag, day, month, i0, i1,
             ...
             fee_t, fee_m, rslip, cap_frac, out_r, out_tr, stats, trades):
    d0 = day[i0]
    E = 1.0
    ...
    for i in range(i0, i1):
        ...
        if pos == 0 and pend != 0 and (not exited):
            if i >= pend_exp:
                ...
                hit = (l[i] <= pend_px - tick) if pend == 1 else (h[i] >= pend_px + tick)
                if hit:
        ...
        if pos != 0 and not exited:
            if pos == 1:
                liq_hit = ml[i] <= mliq
                stop_hit = l[i] <= stop
                tp_hit = (not fill_bar) and h[i] >= tp + tick
                adverse = min(l[i], ml[i])
            else:
                liq_hit = mh[i] >= mliq
                stop_hit = h[i] >= stop
                tp_hit = (not fill_bar) and l[i] <= tp - tick
...
def grid_sim(o, h, l, c, mh, ml, mc, fund, fflag, day, month, i0, i1,
             ...
             fee_t, fee_m, rslip, out_r, out_tr, stats):
    ...
    for i in range(i0, i1):
        ...
        jmax = int(np.floor((A - l[i] - tick) / (g * A) + 1e-9))      # buys at lvl_j fill for j <= jmax
        if jmax > N:
            jmax = N
        nb = jmax - q0 if jmax > q0 else 0
        jmin = int(np.ceil((A - h[i] + tick) / (g * A) - 1e-9))       # sells at lvl_j fill for j >= jmin
        if jmin < -N:
            jmin = -N
    ...
    for i in range(i0, i1):
        ...
        if pos != 0 and not exited:
            if pos == 1:
                liq_hit = ml[i] <= mliq
                stop_hit = l[i] <= stop
                tp_hit = (not entry_bar) and h[i] >= tp + tick
                adverse = min(l[i], ml[i])
            else:
                liq_hit = mh[i] >= mliq
                stop_hit = h[i] >= stop
                tp_hit = (not entry_bar) and l[i] <= tp - tick
                adverse = max(h[i], mh[i])
            if liq_hit:
                liq = True
            elif stop_hit:
