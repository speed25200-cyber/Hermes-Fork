...
from kernels_opt import wick_sim, grid_sim, orb_sim
OPT = int(os.environ.get("OPT", "0"))            # verifier: bitmask of less-pessimistic rules (see kernels_opt.py)
NORS = os.environ.get("NORS", "0") == "1"         # verifier: drop the fast-market range slippage add-on
...
FT, FM, RS = (0.0, 0.0, 0.0) if ZERO else (FEE_T, FEE_M, 0.0 if (NORANGE or NORS) else RANGE_SLIP)
...
def run_sleeve(fam, cfg, coin, L, per):
    ...
    if fam == 'wick':
        ...
        ntr = wick_sim(*args, idx, dr, mf, TICK[coin], MMR[coin], bs, float(L), cfg['entry'] == 'lim', cfg['tp'], cfg['stop'],
                       ...
                       FT, FM, RS, CAP_FRAC, r, tr, st, trades, OPT)
    elif fam == 'grid':
        W = {'fixed': 0.0, 'ema24': 24.0, 'ema168': 168.0}[cfg['centre']]
        grid_sim(*args, TICK[coin], MMR[coin], bs, float(L), cfg['g'], cfg['N'], W, cfg['brk'] == 'stop', FT, FM, RS, r, tr, st, OPT)
    else:
...
if __name__ == '__main__':
    ...
    tag = ('' if MODE == 'main' else MODE + '_') + f'opt{OPT}_nors{int(NORS)}_' + '_'.join(fams)
    if os.environ.get('SMOKE'):
        tag = 'smoke_' + tag
