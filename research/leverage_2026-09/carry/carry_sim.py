...
def sim(coins, L, mode='always', band=0.05, lookback_d=7, theta=0.0, K=2, borrow='okx', stress='mark',
        ...
        if Q.any():
            ...
            if liquidated:
                ...
                if E < 1e-4:
                    ...
                    continue
        if not liquidated:
            Emin_k = min(E, E_prev - float(np.dot(N_prev, JMP[t])) - intr) if (Q.any() and margin == 'joint') else E
        # --- decisions at close ---
    ...
    out.update(liquidations=len(liq_times), trades=trades, rebalances=rebals, imr_violations=imr_viol,
               ...
               liq_dates=';'.join(str(x)[:13] for x in liq_times[:20]))
    if record:
