...
for inst in sel.sym.unique():
    r=sel[sel.sym==inst].iloc[0]; a=int(r.t0); b=a+170*H
    m=candles('history-mark-price-candles',inst,a,b)
    ix=candles('history-index-candles',inst.replace('-SWAP',''),a,b)
    out[inst]=dict(mark=m,index=ix)
    print(inst, 'mark', None if m is None else len(m), 'index', None if ix is None else len(ix), flush=True)
pd.to_pickle(out,'mark_index.pkl')
