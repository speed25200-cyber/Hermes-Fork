...
class Sim:
    ...
    def fpx(self, con, x, buy):
        k = x - con['b0']; F = con['F'][k]
        if self.p.get('fill') == 'worst':
            m = con['Fmc'][k]
            if abs(m / F - 1) > self.p.get('mark_guard', 1.0):
                m = F   # mark unusable (e.g. basis MA not yet built right after listing)
            return max(F, m) if buy > 0 else min(F, m)
        if self.p.get('fill') == 'mark':
            return con['Fmc'][k]
        return F
    ...
    def ppx(self, x, buy):
        ...
        if self.p.get('fill') == 'worst':
            m = self.A['Pmc'][x]
            return max(P, m) if buy > 0 else min(P, m)
        if self.p.get('fill') == 'mark':
            return self.A['Pmc'][x]
        return P
    ...
    def prep(self):
        ...
        with np.errstate(invalid='ignore'):
            okt = TAU >= p['tau_min']
            if p.get('min_age_bars'):
                age = np.array([[d - c['b0'] for d in di] for c in cons])
                okt = okt & (age >= p['min_age_bars'])
    ...
    def run(self):
        ...
        while i < self.s1:
            if pos is None:
                ...
                x = j + self.p.get('delay', 1)  # execution bar
                if x >= self.s1:
                    self.eqc[i:self.s1 + 1] = E; self.eql[i:self.s1 + 1] = E
                    break
                con, sv, sd = found
                q = self.L * E / A['P'][x]
                Fe = self.fpx(con, x, -sd) * (1 - sd * self.slip_fut_open)
                Pe = self.ppx(x, sd) * (1 + sd * c['slip_perp'])
    ...
    def hold(self, pos, i):
        ...
        e_exec = e + self.p.get('delay', 1) if why == 'signal' else e
        ...
        if mode in ('cross', 'cross_stress', 'pm_approx'):
            ...
            mmin = marg.min(0)
            if len(mmin):
                jb = int(np.argmin(mmin))
                if not hasattr(self, 'head'): self.head = []
                self.head.append(dict(con=con['name'], entry=str(GRID[pos['x']]), tmin=str(GRID[t[jb]]), E0=E0,
                                      head_eq=float(mmin[jb] / E0), head_bp=float(mmin[jb] / (q * Pe) * 1e4),
                                      eq_min=float(eqs.min(0)[jb] / E0)))
    ...
    def close_out(self, pos, e, why, cf_end, iso=False):
        ...
        if why == 'expiry':
            ...
            Fx = self.fpx(con, e, sd) * (1 + sd * c['slip_fut']); ffee = q * Fx * c['fee_taker']
            Ptw = self.ppx(e, -sd)
        roll = None
