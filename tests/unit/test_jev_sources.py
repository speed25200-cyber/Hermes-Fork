...
def allowlist(**over) -> SourceAllowlist:
    base = {
        ...
        "sources": [
            {"id": "news_rss", "kind": "rss", "url": "https://news.example.test/feed.xml", "language": "en", "date_method": "rss_pubdate", "poll_interval_seconds": 60},
            {"id": "news_html", "kind": "html", "url": "https://news.example.test/page", "language": "en", "date_method": "page_metadata", "poll_interval_seconds": 60},
            {"id": "news_list", "kind": "html_list", "url": "https://news.example.test/list", "language": "en", "date_method": "page_metadata", "poll_interval_seconds": 60},
            {"id": "status_json", "kind": "json", "url": "https://status.example.test/status.json", "language": "en", "date_method": "declared_field", "poll_interval_seconds": 60, "json_fields": {"items": "incidents", "title": "name", "text": "body", "url": "link", "published_at": "started_at_ms", "guid": "id"}},
            {"id": "partner", "kind": "html", "url": "https://partner.example.test/", "language": "en", "date_method": "absent", "poll_interval_seconds": 60},
        ],
    }
    base.update(over)
...
def fetcher(server: Server, table: dict[str, list[str]], clock: SimulatedClock, **over) -> SafeFetcher:
    return SafeFetcher(allowlist(**over), resolver=resolver_for(table), transport=httpx.MockTransport(server), clock=clock)


PUBLIC = {"news.example.test": [PUBLIC_V4], "status.example.test": [PUBLIC_V4], "partner.example.test": [PUBLIC_V6]}
...
@pytest.mark.parametrize(
    "address",
    ["169.254.169.254", "127.0.0.1", "10.1.2.3", "192.168.1.1", "172.16.5.5", "0.0.0.0", "100.64.0.1", "224.0.0.1",
     "::1", "fd12:3456::1", "fe80::1", "::ffff:127.0.0.1", "::ffff:10.0.0.1", "fd00:ec2::254", "100.100.100.200", "::", "2002:c000:0204::1"],
)
...
async def test_T56_redirects_are_validated_before_being_followed(clock):
    ...
    server = Server(
        {
            "https://news.example.test/to-http": httpx.Response(302, headers={"Location": "http://news.example.test/feed.xml"}),
            "https://news.example.test/to-evil": httpx.Response(301, headers={"Location": "https://evil.test/feed.xml"}),
            "https://news.example.test/to-private": httpx.Response(307, headers={"Location": "https://status.example.test/x"}),
    ...
    assert all("evil.test" not in h and "status.example.test" not in h and not h.startswith("http://") for h in server.hits)
    page = await f.fetch("https://news.example.test/to-ok")
    assert page.final_url == "https://news.example.test/feed.xml" and page.redirects == ("https://news.example.test/to-ok",)
...
async def test_size_limits_apply_to_declared_and_streamed_bodies(clock):
    server = Server(
        {
            "https://news.example.test/big-declared": httpx.Response(200, headers={"Content-Length": "999999"}, content=b"x"),
...
def test_html_parser_is_bounded_in_characters_and_tags():
    ...
    assert "alert" not in item.text and item.title == "Scheduled service interruption" and item.language == "en"
...
<rss><channel><item><title>&lol2;</title></item></channel></rss>"""
    ...
    many = b"<rss><channel>" + b"".join(b"<item><title>t%d</title><description>d</description></item>" % i for i in range(100)) + b"</channel></rss>"
...
<link href="https://news.example.test/atom/1"/><published>2026-09-18T08:00:00+00:00</published><summary>Example Network update.</summary></entry></feed>"""
    ...
    assert entry.url == "https://news.example.test/atom/1" and entry.published_at == datetime(2026, 9, 18, 8, tzinfo=UTC)
    payload = json.dumps({"incidents": [{"id": 7, "name": "Partial outage", "body": "Example Network partial outage.", "link": "https://status.example.test/i/7", "started_at_ms": 1789999200000}, {"id": 8, "name": "No date", "body": "Example Network notice."}]}).encode()
    fields = JsonFieldMap(items="incidents", title="name", text="body", url="link", published_at="started_at_ms", guid="id")
    items = parse_json_items(payload, fields=fields, date_method="declared_field")
    assert items[0].published_at == datetime.fromtimestamp(1789999200, tz=UTC) and items[0].date_method == "declared_field"
...
def test_T58_missing_date_yields_none_never_collection_time(clock):
    ...
    cand = candidate_from_item(source, undated, source_url=source.url, received_at=received, parsed_at=received)
    ...
    assert parse_html_document(HTML_WITH_META, date_method="page_metadata").published_at == datetime(2026, 9, 18, 8, tzinfo=UTC)
...
def _candidate(text: str, received_at: datetime, guid: str = "a1") -> DocumentCandidate:
    source = allowlist().source("news_rss")
    item = ParsedItem(title="Scheduled service interruption", text=text, url=f"https://news.example.test/{guid}", published_at=None, date_method="absent", guid=guid)
    return candidate_from_item(source, item, source_url=source.url, received_at=received_at, parsed_at=received_at)


@pytest.mark.parametrize("backend", ["memory", "sqlite"])
def test_content_change_creates_a_new_version_same_identity(backend):
    sink = MemoryDocumentSink() if backend == "memory" else DocumentStore(make_session_factory(memory_engine()))
...
async def test_T55_prompt_injection_in_a_document_is_data_only(clock):
    ...
    collector = SourceCollector(allowlist(), fetcher=fetcher(server, PUBLIC, clock), sink=sink, registry=registry, clock=clock)
    ...
    build = build_request(model=questions.model, question_set=questions, canonical_name="Example Network", symbol="EXM", title=doc.title, text=doc.text, max_document_characters=20000)
    payload = build.request.payload()
    assert set(payload) == {"model", "state", "questions"} and set(payload["state"]) == {"asset", "document", "prior_documents"}
    assert payload["questions"] == {k: v.model_dump(mode="json", exclude_none=True) for k, v in questions.questions.items()}
...
async def test_collector_html_list_depth_one_same_host_only_and_disabled_source(clock):
    server = Server(
        {
            "https://news.example.test/list": httpx.Response(200, content=b'<html><body><a href="/item/1">one</a><a href="https://evil.test/x">evil</a><a href="/item/2">two</a></body></html>'),
    ...
    collector = SourceCollector(allowlist(), fetcher=fetcher(server, PUBLIC, clock), sink=MemoryDocumentSink(), clock=clock, max_items=10)
    result = await collector.poll("news_list")
    assert len(result.documents) == 2 and "https://evil.test/x" not in server.hits
    assert result.documents[0].published_at == datetime(2026, 9, 18, 8, tzinfo=UTC) and result.documents[1].published_at is None
    assert extract_links(b'<a href="/x">x</a><a href="https://evil.test/">e</a>', base_url="https://news.example.test/") == [("https://news.example.test/x", "x")]
    disabled = allowlist(sources=[{"id": "off", "kind": "rss", "url": "https://news.example.test/f", "date_method": "absent", "poll_interval_seconds": 60, "enabled": False}])
    collector2 = SourceCollector(disabled, fetcher=fetcher(server, PUBLIC, clock), sink=MemoryDocumentSink(), clock=clock)
